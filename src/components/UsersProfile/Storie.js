...
const HighLight = ({userid , storie , date , open , handleDialogClose1 , setOpen1 , length}) => {
...
//   const [open ,setOpen] = useState(false)
  ...
  useEffect(()=>{
   ...
   useEffect(()=>{
             setArray(storie?.slice(0 , storie?.length -1))
   },[storie])
    return (
        <>
        <Dialog
             ...
             <div className='story__item__click' >
               <div className='story__indicator'>
                  ...
                  <div className='story__info'>
                    ...
                    {storie && <span> {moment(storie[activeStep]?.time?.toDate()).fromNow()}</span>}
                   ...
                   >
                   {
                    storie?.map((step , index)=>(
                     <div>
                         ...
                         (<img className={classes.img} src={step.storyphoto}/>) : console.log('stop')}
                     </div>
                    ))
                   }
                  </AutoPlaySwipeableViews>
